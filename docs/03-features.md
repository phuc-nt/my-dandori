...
| G1.5 | **Secret/PII guardrail** — secret (API key, private key) trong tool-call → **Deny**; PII (thẻ Luhn-valid, ≥5 email) → **Gate** (chờ duyệt); mask trước khi vào DB/Slack | "Agent dán API key/số thẻ vào lệnh — chặn ngay, không rò vào log" | leak secret/PII qua agent log là control-gap thật | **MVP ✅** |
...
| G3 | **Budget circuit-breaker** — trần token/đô theo agent · project; cảnh báo 50/75/90% → mặc định **downgrade-gate** (vượt trần chỉ Deny nếu run đang chạy model đắt, kèm gợi ý `/model`; model rẻ chạy tiếp), `budget.mode: hard` phục hồi hard-stop cũ | "Vượt hạn mức → dừng, không đợi cuối tháng giật mình" | spend dễ scale, khó forecast | **[MVP]** |
...
| L10 | **Practice mining** (`/knowledge/mining`) — 4 tín hiệu on-demand (corrective-steering≥2, fail-retry-success, guardrail-block-then-done, cost-outlier >3× median project) tìm "run đáng đọc", không phải leaderboard | "Run nào vừa rồi đáng để cả team học theo — không cần chờ ai nhớ ra kể lại" | tri thức tốt trôi qua vì không ai đủ rảnh lục lại run cũ | **v13 ✅** |
| L11 | **AI-draft assistant** — LLM sinh nháp practice markdown từ evidence DB (redacted, không đọc transcript) cho một run mined, người sửa rồi mới nominate | "Từ run đáng đọc ra một bản nháp tri thức trong 1 click, đỡ công viết tay" | soạn playbook tay chậm, ít ai làm | **v13 ✅** (fail-open: thiếu key → không có nút; OpenRouter down → form rỗng vẫn dùng được) |
| L12 | **Kit bundle** — người tự chọn `.claude/{agents,rules,skills,commands}/*.md` đã git-track, đóng gói thành 1 unit `kind=kit` xét-duyệt-nguyên-khối, phân phối qua `kit pull` (hash-pin + deny-list path-safety) | "Nhân bản đúng bộ agent/rule/skill mà team A đang dùng tốt sang team B, không copy tay từng file" | context/skill tốt bị khoá trong một repo, không portable | **v13 ✅** |
| L13 | **Import memory/journal** — `dandori knowledge import` đưa `.md` có sẵn (CLAUDE.md memory, journal) vào `kind=context` qua review gate thay vì nằm ngoài mọi kiểm soát | "File tri thức tay đã viết từ trước, đưa vào vòng review-publish luôn, không viết lại" | tri thức cũ nằm rải rác, không version, không audit | **v13 ✅** |
...
**GOVERN** G1 pre-action block · G1.5 secret/PII guardrail · G2 sandbox · G3 budget breaker · G4 permission gate · G7 quality gate
