...
### OpenRouter — thêm `OPENROUTER_MODEL` (v13, bắt buộc cho AI-draft)

- **AI-draft** (`/knowledge/mining` → nút draft) cần cả `OPENROUTER_API_KEY` **và** `OPENROUTER_MODEL` trong `.env`. Thiếu `OPENROUTER_MODEL` → hard-fail với thông điệp "chưa cấu hình model" — Dandori cố tình **không tự chọn model hộ người dùng**.
- Nút draft chỉ hiện khi cả hai key có mặt; OpenRouter down lúc bấm → trả fragment lỗi, form nominate rỗng vẫn dùng tay bình thường (fail-open, không chặn nominate thủ công).
...
## 3d. Kit & import (v13) — local-mode CLI, không phải service integration

- `dandori kit nominate/pull` và `dandori knowledge import` **không nối service ngoài nào** — chạy hoàn toàn local-mode qua CLI, đọc/ghi trực tiếp `.claude/` và SQLite của máy đang chạy. Không cần token gì thêm ngoài các mục ở trên.
- `kit nominate` chỉ đóng gói file **đã git-track** dưới `.claude/{agents,rules,skills,commands}` — file chưa `git add` sẽ không xuất hiện trong kit (không phải bug, là kỷ luật "commit trước").
- `kit pull` ghi file vào `.claude/` của repo hiện tại — chạy trong repo bạn muốn nhận kit, không phải repo nguồn.
