...
**Điều kiện mở plan v10 (metric overhaul):** density đạt — duration dương đa số ✓ (30/37), task_key có JOIN ✓ (dù thấp), steering text tồn tại ✓ (29 run), guardrail leg live-PASS ✓. **Nhưng** task_key coverage + git-delta lịch sử vẫn mỏng (data mất, không backfill được) → v10 phải chờ tích luỹ run MỚI (đã cài hook) trước khi reweight composite; reweight phải show before/after ranking delta thật, delta ~0 thì hoãn (YAGNI).
